...
export const HoverEffect = ({
    ...
    const handleSetSelectedEvent = (item: { title: string; description: string; image: string }) =>{
            ...
            {items.map((item, idx) => (
                ...
                >
                    <AnimatePresence>
                        {hoveredIndex === idx && (
                            <motion.span
                                className="absolute inset-0 h-full w-full bg-neutral-200 dark:bg-white/[0.2] block  rounded-3xl"
...
export const Card = ({
    ...
    return (
        <div
            className={cn(
                "rounded-2xl  h-full max-h-[200px] w-full p-4  bg-[#00000078] bg-blend-multiply border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20",
                className
            )}
...
export const CardTitle = ({
    ...
    return (
        <h4 className={cn("text-zinc-100 sm:text-sm 2xl:text-2xl font-bold tracking-wide mt-4", className)}>
            {children}
        </h4>
    );
};
